...
export default async function WatchPage({ params }: { params: Params }) {
  ...
  const remainingSeconds = Math.max(0, row.access.allowedSeconds - row.access.usedSeconds);
  ...
  if (!youtubeId) {
    return (
      <main className="mx-auto max-w-4xl px-6 py-10">
        <p className="rounded-lg bg-red-50 p-4 text-red-700">Video source is invalid. Please inform your teacher.</p>
      </main>
    );
  }
