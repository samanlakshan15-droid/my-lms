...
export async function grantVideoAccessAction(formData: FormData) {
  ...
  if (!existing) {
    ...
    let nextWatchStartedAt = existing.watchStartedAt;

    if (existing.expiresAt) {
      if (existing.expiresAt.getTime() > now.getTime()) {
        nextExpiresAt = new Date(existing.expiresAt.getTime() + extraSeconds * 1000);
      } else {
        nextWatchStartedAt = now;
        nextExpiresAt = new Date(now.getTime() + extraSeconds * 1000);
      }
    }
    ...
    await db
      ...
      .set({
        ...
        watchStartedAt: nextWatchStartedAt,
