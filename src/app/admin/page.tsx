            ...
            <div className="space-y-2">
              {teacherRows.map((teacher) => (
                <form key={teacher.id} action={removeTeacherAccountAction} className="flex items-center justify-between rounded-lg border border-slate-200 p-3">
                  ...
                  <ConfirmSubmitButton
                    label="Remove"
                    confirmMessage={`Are you sure you want to remove teacher login '${teacher.username}'?`}
      ...
      <section className="mb-6 rounded-2xl bg-white/90 p-5 shadow-lg backdrop-blur">
        ...
        {videoRows.length === 0 ? (
          ...
          <div className="space-y-3">
            {videoRows.map((video) => (
              <div key={video.id} className="grid gap-2 rounded-xl border border-slate-200 p-3 md:grid-cols-[80px_1fr_1fr_auto_auto] md:items-center">
                <form action={updateVideoAction} className="contents">
                ...
                <form action={removeVideoAction}>
                  ...
                  <ConfirmSubmitButton
                    label="Remove Video"
                    confirmMessage={`Remove video '${video.title}'? This will also remove all related user access.`}
                    className="rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white"
                  />
                </form>
              </div>
                ...
                <article key={user.id} className="rounded-xl border border-slate-200 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <form action={removeUserAction}>
                      ...
                      <ConfirmSubmitButton
                        label="Remove User"
                        confirmMessage={`Remove user '${user.fullName}' permanently?`}
                        className="rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white"
                      />
                    </form>
                  </div>
                        ...
                        return (
                          <div key={access.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-slate-50 p-2">
                            <p className="text-sm text-slate-700">
                              {video?.title ?? `Video #${access.videoId}`} - allocated {Math.ceil(access.allowedSeconds / 60)} min, remaining {Math.ceil(Math.max(0, access.allowedSeconds - access.usedSeconds) / 60)} min
                            </p>
                            <form action={removeVideoAccessAction}>
                              <input type="hidden" name="accessId" value={access.id} />
                              <ConfirmSubmitButton
                                label="Remove Access"
                                confirmMessage="Are you sure you want to remove this video access?"
